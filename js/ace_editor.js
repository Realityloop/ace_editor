      ...
      if (Drupal.settings.ace_editor.editor_instances.length) {
        $(Drupal.settings.ace_editor.editor_instances).each(function (i) {
          ...
          if (height.toLowerCase() === 'auto') {
            ...
            // Create the hidden span, append it to the PRE and get its width
            // with nowrap.
            var content_width = $('<span></span>').attr('id', 'ace_editor_to_remove').css({
              display: 'none',
              whiteSpace: 'nowrap'
            }).appendTo($('#' + editor_instance.container.id)).text(editor_instance.getSession().getValue()).width();
